...
vector<vector<int>> attributes(3, vector<int>(7));
int counter = 0;
...
vector<vector<int>> operator_swap(vector<vector<int>> sol) {
	...
	int airport1 = sol[1][loc1];
	int airport2 = sol[1][loc2];
	...
	attributes[0][0] = loc1;
	attributes[0][1] = loc2;
	attributes[0][2] = sol[0][loc1];
	attributes[0][3] = sol[0][loc2];
	attributes[0][4] = airport1;
	attributes[0][5] = airport2;
	attributes[0][6] = 0;
...
vector<vector<int>> operator_insert(vector<vector<int>> sol) {
	...
	int airport1 = sol[1][loc];
	int airport2 = sol[1][index];

	if (rand() % 2 == 0) {
		sol[1][loc] = rand() % airport_in_area_index[sol[0][loc]].size();
		airport1 = sol[1][loc];
	}
	if (rand() % 2 == 0) {
		sol[1][index] = rand() % airport_in_area_index[sol[0][index]].size();
		airport2 = sol[1][index];
	}

	attributes[1][0] = loc;
	attributes[1][1] = index;
	attributes[1][2] = sol[0][loc];
	attributes[1][3] = sol[0][index];
	attributes[1][4] = airport1;
	attributes[1][5] = airport2;
	attributes[1][6] = 1;

	calculate_feas_partial(loc, sol);
	calculate_feas_partial(index, sol);
...
vector<vector<int>> operator_reverse(vector<vector<int>> sol) {
	...
	attributes[2][0] = loc1;
	attributes[2][1] = loc2;
	attributes[2][2] = sol[0][loc1];
	attributes[2][3] = sol[0][loc2];
	attributes[2][4] = airport1;
	attributes[2][5] = airport2;
	attributes[2][6] = 2;
...
vector<vector<int>> ImproveInitialSolution(vector<vector<int>> sol) {
	...
	vector<vector<int>> sol_temp, sol_temp_prev = sol;
	vector<vector<int>> tabu_list(number_of_areas, vector<int>(7));//*******************************************************
	...
	while (((float)(clock() - start)) / CLOCKS_PER_SEC <= totalTime) {
		//applying the 3 operators
		//record their attributes
		
		//swap(0):insert(1):reverse(2) ->
		//small files  --->   /*swap and reverse*/
		//medium files  --->  /*insert*\
		//large files  --->   /*swap*/

		int i;
		int min_cost = -1, temp_cost;
		int selected_index;

		for (int h = 0; h < 11; h++) {//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			if (h < 5) {
				sol_temp = ApplyOperator(sol, 0);
				i = 0;
			}
			if (h >= 7) {
				sol_temp = ApplyOperator(sol, 2);
				i = 2;
			}
			else {
				sol_temp = ApplyOperator(sol, 1);
				i = 1;
			}
			...
			if (m_non_feas.size() == 0) {
				temp_cost = TotalCost(sol_temp);
				if (min_cost == -1 || temp_cost < min_cost) {
					min_cost = temp_cost;
					selected_index = i;
					sol_temp_prev = sol_temp;
					map_temp = m_non_feas;
				}
				else {
					sol_temp = sol_temp_prev;
				}
			}
			else {
				sol_temp = sol_temp_prev;
		...
		if (min_cost != -1) {
			...
			//check if it does not exist in tabu list
			bool exist = false;
			vector<int> check;
			for (int i = 0; i < counter; i++) {
				check = { 0,2,3,4,5,6,7 };
				for (int j = 0; j < tabu_list[i].size(); j++) {
					if (tabu_list[i][j] == attributes[selected_index][j]) {
						check[j] = 1;
					}
				}
				if (equal(check.begin() + 1, check.end(), check.begin())) {//if all ones then exists
					exist = true;
					break;
				}
			}

			//accept if it does not exist in the tabu list
			if (exist == false) {
				//accept the solution
				if (accepted == false) {
					sol_prev = CopyFromTo_sol(sol, sol_prev);
				...
				// for tabu-move :--->
				swap(attributes[selected_index][0], attributes[selected_index][1]);
				swap(attributes[selected_index][2], attributes[selected_index][3]);
				swap(attributes[selected_index][4], attributes[selected_index][5]);

				//insert in the list
				if (counter >= number_of_areas) {
					tabu_list.erase(tabu_list.begin());
					tabu_list.push_back(attributes[selected_index]);
				}
				else {
					tabu_list[counter] = attributes[selected_index];
					counter++;
				}
