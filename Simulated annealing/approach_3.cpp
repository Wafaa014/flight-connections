...
vector<vector<int>> ImproveInitialSolution(vector<vector<int>> sol) {
	...
	//initial temp list
	int i = 0;
	vector<float> initial_temp_list(100);
	float p_0 = 0.9;
	float t;
	while (i < initial_temp_list.size() &&
		((float)(clock() - start)) / CLOCKS_PER_SEC <= totalTime)
	{
		int h = rand() % number_of_operators;
		...
		t = -(abs(cost - cost_prev)) / (log(p_0));
		initial_temp_list[i] = t;
		i++;
	}

	while (((float)(clock() - start)) / CLOCKS_PER_SEC <= totalTime) {

			float t_max = initial_temp_list[0];
			for (int j = 0; j < initial_temp_list.size(); j++) {
				if (initial_temp_list[j] > t_max)
					t_max = initial_temp_list[j];
			}
			t = 0; int c = 0; int m = 0;
			while (m < number_of_areas)
			{
				int h = rand() % number_of_operators;
				sol = ApplyOperator(sol, h);
				m++;
				if (m_non_feas.size() == 0) {
					...
					if (cost <= cost_prev) {
						...
						float p = exp(-(cost - cost_prev) / t_max);
						float r = static_cast<float> (rand()) / static_cast<float> (RAND_MAX);
						if (r < p) {
							t = (t - (cost - cost_prev)) / log(r);
							c++;
							//accept sol
							sol_prev = CopyFromTo_sol(sol, sol_prev);
							cost_prev = cost;
			...
			if (c != 0) {
				initial_temp_list.erase(find(initial_temp_list.begin(),
					initial_temp_list.end(), t_max));
				initial_temp_list.insert(initial_temp_list.end(), t/c);
			}
	}
